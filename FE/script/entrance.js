const masukKelas = async () => {
  ...
  window.location =
    urlPart.splice(0, urlPart.length - 1).join("/") + "/entrance.html";
};
...
const increaseCounter = async () => {
  ...
  let result = await fetch(
    `http://52.76.55.94:3000/api/v1/kelas/counter/${id}/increase`,
    {
      method: "PUT",
      headers: {
...
const decreaseCounter = async () => {
  ...
  console.log(id);
  let result = await fetch(
    `http://52.76.55.94:3000/api/v1/kelas/counter/${id}/decrease`,
    {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
...
const showCounter = async () => {
  let id = window.localStorage.getItem("idMasuk");
  let result = await fetch(
    `http://52.76.55.94:3000/api/v1/kelas/counter/${id}/show`,
    {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
